...
- `"How is the weather in Hangzhou?"` (Triggers `get_weather`)
- `"Do a backflip"` (Triggers `perform_move`)
- `"Check battery and device state"` (Triggers `get_battery_status` & `get_device_state`)
- `"Play some music"` (Triggers `play_sound`)
